...
def bn_init_as_tf(m):
    if isinstance(m, nn.BatchNorm2d):
        m.track_running_stats = True  # 从预训练模型加载信息 These two lines enable using stats (moving mean and var) loaded from pretrained model
        m.eval()  # 或者如果批次范数图层没有预先训练值，则均值和方差为零 or zero mean and variance of one if the batch norm layer has no pretrained values
        m.affine = True
        m.requires_grad = True


def weights_init_xavier(m):  # 权重初始化
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):  # params参数 feat_out_channels特征输出通道  特征数
        ...
        self.daspp_3 = atrous_conv(num_features // 2, num_features // 4, 3, apply_bn_first=False)  # 带洞的卷积 空洞卷积
    ...
    def forward(self, features, focal):
        ...
        if self.params.dataset == 'kitti':
            final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873  #743
